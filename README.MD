...
Mi idea principal era que cuando el usuario hiciera clic en "Editar", el formulario de "Agregar nuevo usuario" se transformara en un formulario de "Editar usuario", se rellenara con los datos de la persona seleccionada y, al enviarlo, actualizara la información en el servidor.
...
1.  **Usuario hace clic en "Editar"**: La pagina recoge los datos del usuario
2.  **Servidor responde**: El servidor busca los datos y se los devuelve
...
5.  **La página informa al servidor**: La página llama de nuevo al servidor y le dice: "Actualiza al usuario de la fila 3 con este nuevo nombre.
6.  **Servidor confirma**: El servidor guarda el cambio y me responde
7.  **La página se refresca**: La tabla se redibuja con la nueva información.

---

## Parte 1: El Frontend (Lo que ve el usuario)

Hice los cambios principales en mi fichero `assets/js/main.js`.

### 1. Añadir el botón "Editar" a cada fila

Lo primero fue modificar la función `renderizarTablaDeUsuarios`. Dentro del bucle que recorre los usuarios, donde construyo el HTML de cada fila, añadí un nuevo botón. A este botón le puse la clase `boton-editar` y un atributo `data-posicion` al que le asigno la posición del usuario en la lista. 
...
Link al github: https://github.com/nicolasalvo/CRUD-Asincrono.git